...
@bot.slash_command()
async def badge(ctx):
	print(f'[[[slash command "/badge" used]]]\n[[[user: {ctx.user}]]]\n[[[guild: {ctx.guild}]]]\n')
	
	await ctx.respond(content="https://discord.com/developers/active-developer", ephemeral=True)
...
@bot.command(aliases=["code", "run"])
@commands.is_owner()
async def _code(ctx, c, *, data):
	if c.startswith("ev"):
		try:
			r = eval(data)
		except:
...
def converttobuttons(buttons: str):
...
@bot.slash_command(description="echo (/embed replacement). os.echo for more info.")
...
async def echo(
	...
	buttons: Option(str, "UNSTABLE. message buttons. check os.echo for more info.", required=False, default=None),
...
@bot.command(aliases=["shutdown", "sd"])
async def _shutdown(ctx):
	await ctx.send("`closing connection...`")
	await bot.close()

if test:
	bot.run("OTM1MTU5MjYyMDAxMzkzNjY0.GMWAEH.fwTMnVGLz9pFevQSvnyovqd5eLa7nidBIhF1vw")
else:
	bot.run("OTI3MTYzMDAzNjM4NTQ2NDQy.G_hbdS.Uy_KWCfsXulFbDaQBfaCV1l3BpRSMkXGu9ZnyE")
